...
try:
  ...
  def project_points_cuda(p3d, k, c, f):
...
def project_points_numpy(p3d, k, c, f):
  p2d = np.zeros((2,p3d.shape[1]))
  for i in range(p3d.shape[1]):
    ...
    p2d[0,i] = R*p[0]/rp + c[0]
    p2d[1,i] = R*p[1]/rp + c[1]
  return p2d


def project_points_fisheye(p3d, k, c, f):
  if USE_CUDA:
    p2d = project_points_cuda(p3d, k, c, f)
  else:
    p2d = project_points_numpy(p3d, k, c, f)
  return p2d


if __name__ == "__main__":
    p3d = np.random.randn(3,4)
    x2d = project_points_fisheye(p3d, [0,0,0,0], [1224,1024], 750.1)
    print(x2d)
    x2d = project_points_numpy(p3d, [0,0,0,0], [1224,1024], 750.1)
    print(x2d)